...
BEGIN
  ...
  WHEN '/card/set' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN EXECUTE format('SELECT api.set_card(%s) AS id FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_card', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_card', 'api', true), ', ')) USING pPayload
      LOOP
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT api.set_card(%s) AS id FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_card', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_card', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT row_to_json(r);
      END LOOP;
