...
BEGIN
  ...
  WHEN '/device/method' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT r.id, api.get_method(GetObjectClass(r.id), GetObjectState(r.id)) as method FROM api.get_device(r.id) ORDER BY id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/device/transaction/get' THEN
  ...
  WHEN '/device/transaction/list' THEN
  ...
  WHEN '/device/meter_value/get' THEN
  ...
  WHEN '/device/meter_value/list' THEN
