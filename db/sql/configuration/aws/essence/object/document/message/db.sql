 ...
 * @param {id} pParent - Родительский объект
 * @param {id} pType - Тип
 * @param {text} pFrom - От
 * @param {text} pTo - Кому
...
BEGIN
  ...
  IF nClass IS null OR GetClassCode(nClass) <> 'message' THEN
    PERFORM IncorrectClassType();
  END IF;

  nDocument := CreateDocument(pParent, pType, pSubject, pDescription);

  INSERT INTO db.message (document, address_from, address_to, subject, body)
  VALUES (nDocument, pFrom, pTo, pSubject, pBody)
...
BEGIN
  ...
  UPDATE db.message 
     SET address_from = coalesce(pFrom, address_from),
         address_to = coalesce(pTo, address_to),
...
CREATE OR REPLACE VIEW Message (Id, Document, Code, AddressFrom, AddressTo, 
  Subject, Body
)
AS
  SELECT id, document, code, address_from, address_to, subject, body
    FROM db.message;
