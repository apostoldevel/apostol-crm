 ...
 * @return {uuid}
 */
CREATE OR REPLACE FUNCTION CreateMode (
  pParent       uuid,
  pType         uuid,
  pCode         text,
  pName         text,
 ...
 * @return {void}
 */
CREATE OR REPLACE FUNCTION EditMode (
  pId           uuid,
...
CREATE OR REPLACE FUNCTION GetMode (
  pCode		text
) RETURNS 	uuid
AS $$
BEGIN
  RETURN GetReference(pCode, 'mode');
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
