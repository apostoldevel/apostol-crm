...
--------------------------------------------------------------------------------
/**
 * @brief Dispatches REST API requests for the invoice entity
 * @param {text} pPath - REST route path
 * @param {jsonb} pPayload - JSON
 * @return {SETOF json} - Records as JSON
 * @since 1.0.0
 */
CREATE OR REPLACE FUNCTION rest.invoice (
  pPath       text,
...
BEGIN
  ...
  WHEN '/invoice/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEntity('invoice')
      LOOP
  ...
  WHEN '/invoice/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_invoice(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/invoice/set' THEN

    IF pPayload IS NULL THEN
      PERFORM JsonIsEmpty();
    END IF;

    arKeys := array_cat(arKeys, GetRoutines('set_invoice', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN EXECUTE format('SELECT row_to_json(api.set_invoice(%s)) FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_invoice', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_invoice', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT row_to_json(api.set_invoice(%s)) FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_invoice', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_invoice', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
