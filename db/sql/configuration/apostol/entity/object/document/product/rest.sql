...
--------------------------------------------------------------------------------
/**
 * @brief Dispatches REST API requests for the product entity
...
BEGIN
  ...
  WHEN '/product/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_product(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/product/list' THEN
    ...
    LOOP
      IF r.groupby IS NOT NULL THEN
        FOR e IN EXECUTE format('SELECT %s FROM api.list_product($1, $2, $3, $4, $5) GROUP BY %s', array_to_string(array_quote_literal_json(JsonbToStrArray(r.fields)), ','), array_to_string(array_quote_literal_json(JsonbToStrArray(r.groupby)), ',')) USING r.search, r.filter, r.reclimit, r.recoffset, r.orderby
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      ELSE
