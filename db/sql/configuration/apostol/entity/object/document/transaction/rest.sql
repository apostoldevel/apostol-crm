...
--------------------------------------------------------------------------------
/**
 * @brief Dispatches REST API requests for the transaction entity
...
BEGIN
  ...
  WHEN '/transaction/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_transaction(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
