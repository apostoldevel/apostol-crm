...
--------------------------------------------------------------------------------
/**
 * @brief Dispatches REST API requests for the tariff entity
 * @param {text} pPath - REST route path
 * @param {jsonb} pPayload - JSON
 * @return {SETOF json} - Records as JSON
 * @since 1.0.0
 */
CREATE OR REPLACE FUNCTION rest.tariff (
  pPath       text,
...
BEGIN
  ...
  WHEN '/tariff/type' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEntity('tariff')
      LOOP
  ...
  WHEN '/tariff/set' THEN

    IF pPayload IS NULL THEN
      PERFORM JsonIsEmpty();
    END IF;

    arKeys := array_cat(arKeys, GetRoutines('set_tariff', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);
  ...
  WHEN '/tariff/scheme/get' THEN

    IF pPayload IS NULL THEN
      PERFORM JsonIsEmpty();
    END IF;

    arKeys := array_cat(arKeys, ARRAY['service', 'servicecode', 'currency', 'currencycode', 'tag', 'fields']);
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(service uuid, servicecode text, currency uuid, currencycode text, tag text, fields jsonb)
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_tariff_scheme($1, $2, $3)', JsonbToFields(r.fields, GetColumns('tariff_scheme', 'api'))) USING coalesce(r.service, GetService(r.servicecode)), coalesce(r.currency, GetCurrency(coalesce(r.currencycode, 'USD'))), coalesce(r.tag, 'default')
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN SELECT * FROM jsonb_to_record(pPayload) AS x(service uuid, servicecode text, currency uuid, currencycode text, tag text, fields jsonb)
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_tariff_scheme($1, $2, $3)', JsonbToFields(r.fields, GetColumns('tariff_scheme', 'api'))) USING coalesce(r.service, GetService(r.servicecode)), coalesce(r.currency, GetCurrency(coalesce(r.currencycode, 'USD'))), coalesce(r.tag, 'default')
        LOOP
          RETURN NEXT row_to_json(e);
  ...
  WHEN '/tariff/scheme/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      FOR r IN SELECT * FROM jsonb_to_recordset(pPayload) AS x(search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
      LOOP
        FOR e IN SELECT count(*) FROM api.list_tariff_scheme(r.search, r.filter, r.reclimit, r.recoffset, r.orderby)
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/tariff/scheme/list' THEN
    ...
    FOR r IN SELECT * FROM jsonb_to_record(pPayload) AS x(fields jsonb, search jsonb, filter jsonb, reclimit integer, recoffset integer, orderby jsonb)
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.list_tariff_scheme($1, $2, $3, $4, $5)', JsonbToFields(r.fields, GetColumns('tariff_scheme', 'api'))) USING r.search, r.filter, r.reclimit, r.recoffset, r.orderby
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;
