...
/**
 * @brief API view for model records.
 * @since 1.0.0
 */
 ...
 * @param {uuid} pVendor - Vendor identifier
...
AS $$
BEGIN
  RETURN CreateModel(pParent, coalesce(pType, GetType('unknown.model')), pVendor, pCategory, pCode, pName, pDescription);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
...
AS $$
BEGIN
  PERFORM FROM db.model t WHERE t.id = pId;

  IF NOT FOUND THEN
    PERFORM ObjectNotFound('model', 'id', pId);
  END IF;

  PERFORM EditModel(pId, pParent, pType, pVendor, pCategory, pCode, pName, pDescription);
END;
$$ LANGUAGE plpgsql
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
...
/**
 * @brief Creates or updates a model (upsert).
 * @return {SETOF api.model} - Updated model record
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Returns a model by identifier.
 * @param {uuid} pId - Identifier
 * @return {api.model} - Model record
 * @since 1.0.0
 */
...
AS $$
  SELECT * FROM api.model WHERE id = pId AND CheckObjectAccess(id, B'100')
$$ LANGUAGE SQL
   SECURITY DEFINER
   SET search_path = kernel, pg_temp;
...
--------------------------------------------------------------------------------
/**
 * @brief Returns a list of models.
 ...
 * @return {SETOF api.model} - Model records
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief API view for model properties.
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Sets model properties from JSON array.
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Sets model properties from JSONB array.
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Returns model properties as JSON.
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Returns model properties as JSONB.
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Sets a model property value.
 * @param {uuid} pModel - Model identifier
 * @param {uuid} pProperty - Property identifier
 * @return {SETOF api.model_property} - Model property record
 * @since 1.0.0
 */
...
--------------------------------------------------------------------------------
/**
 * @brief Returns a model property.
 * @param {uuid} pModel - Model identifier
 * @param {uuid} pProperty - Property identifier
 * @return {SETOF api.model_property} - Model property record
 * @since 1.0.0
...
--------------------------------------------------------------------------------
/**
 * @brief Deletes a model property.
 * @param {uuid} pModel - Model identifier
 * @param {uuid} pProperty - Property identifier
 * @return {boolean}
 * @since 1.0.0
...
--------------------------------------------------------------------------------
/**
 * @brief Deletes all properties of a model.
 * @param {uuid} pModel - Model identifier
 * @return {void}
 * @since 1.0.0
 */
 ...
 * @param {integer} pOffSet - Number of rows to skip
 * @param {jsonb} pOrderBy - Sort by fields array
 * @return {SETOF api.model_property} - Model record
 * @since 1.0.0
 */
