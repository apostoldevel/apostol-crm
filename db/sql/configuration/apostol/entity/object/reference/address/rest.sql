...
--------------------------------------------------------------------------------
/**
 * @brief REST JSON API endpoint for address operations.
...
BEGIN
  ...
  WHEN '/address/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_address(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
