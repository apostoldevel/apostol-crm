...
/**
 * @brief Category reference view with type, scope, and localized text.
 * @since 1.0.0
 */
...
/**
 * @brief Access control view for category (RLS). Returns object IDs accessible to the current user.
 * @since 1.0.0
 */
...
WITH _membergroup AS (
  SELECT current_userid() AS userid UNION SELECT userid FROM db.member_group WHERE member = current_userid()
	...
	FROM db.category t INNER JOIN db.aou         a ON a.object = t.id
...
/**
 * @brief Full category object view with entity, class, type, state, owner, and scope details.
 * @since 1.0.0
 */
