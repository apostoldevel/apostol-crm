...
/**
 * @brief Region reference view with type, scope, and localized text.
 * @since 1.0.0
 */
CREATE OR REPLACE VIEW Region (Id, Reference,
  ...
  SELECT t.id, t.reference,
    ...
    FROM db.region t INNER JOIN db.reference         r ON t.reference = r.id AND r.scope = current_scope()
...
/**
 * @brief Access control view for region (RLS). Returns object IDs accessible to the current user.
 * @since 1.0.0
 */
...
WITH _membergroup AS (
  SELECT current_userid() AS userid UNION SELECT userid FROM db.member_group WHERE member = current_userid()
    ...
    FROM db.region t INNER JOIN db.aou         a ON a.object = t.id
...
/**
 * @brief Full region object view with entity, class, type, state, owner, and scope details.
 * @since 1.0.0
 */
