...
BEGIN
  ...
  CASE pPath
  WHEN '/region/type' THEN

    FOR r IN SELECT * FROM jsonb_to_record(pPayload) AS x(fields jsonb)
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.type($1)', JsonbToFields(r.fields, GetColumns('type', 'api'))) USING GetEntity('region')
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;

  WHEN '/region/method' THEN
  ...
  WHEN '/region/count' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT * FROM api.count_format(r.search, r.filter) AS count
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/region/set' THEN

    IF pPayload IS NULL THEN
      PERFORM JsonIsEmpty();
    END IF;

    arKeys := array_cat(arKeys, GetRoutines('set_region', 'api', false));
    PERFORM CheckJsonbKeys(pPath, arKeys, pPayload);

    IF jsonb_typeof(pPayload) = 'array' THEN

      FOR r IN EXECUTE format('SELECT row_to_json(api.set_region(%s)) FROM jsonb_to_recordset($1) AS x(%s)', array_to_string(GetRoutines('set_region', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_region', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
    ...
    ELSE
      ...
      FOR r IN EXECUTE format('SELECT row_to_json(api.set_region(%s)) FROM jsonb_to_record($1) AS x(%s)', array_to_string(GetRoutines('set_region', 'api', false, 'x'), ', '), array_to_string(GetRoutines('set_region', 'api', true), ', ')) USING pPayload
      LOOP
        RETURN NEXT r;
      END LOOP;
  ...
  WHEN '/region/get' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN EXECUTE format('SELECT %s FROM api.get_region($1)', JsonbToFields(r.fields, GetColumns('region', 'api'))) USING r.id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
  ...
  WHEN '/region/list' THEN
    ...
    LOOP
      FOR e IN EXECUTE format('SELECT %s FROM api.list_region($1, $2, $3, $4, $5)', JsonbToFields(r.fields, GetColumns('region', 'api'))) USING r.search, r.filter, r.reclimit, r.recoffset, r.orderby
      LOOP
        RETURN NEXT row_to_json(e);
      END LOOP;
    END LOOP;
