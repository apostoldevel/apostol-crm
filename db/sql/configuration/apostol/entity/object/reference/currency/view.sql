...
/**
 * @brief Currency reference view with type, scope, and localized text.
 * @since 1.0.0
 */
CREATE OR REPLACE VIEW Currency (Id, Reference,
  ...
  SELECT t.id, t.reference,
    ...
    FROM db.currency t INNER JOIN db.reference         r ON t.reference = r.id AND r.scope = current_scope()
...
/**
 * @brief Access control view for currency (RLS). Returns object IDs accessible to the current user.
 * @since 1.0.0
 */
...
WITH _membergroup AS (
  SELECT current_userid() AS userid UNION SELECT userid FROM db.member_group WHERE member = current_userid()
	...
	FROM db.currency t INNER JOIN db.aou         a ON a.object = t.id
...
/**
 * @brief Full currency object view with entity, class, type, state, owner, and scope details.
 * @since 1.0.0
 */
...
AS
  SELECT t.id, r.object, o.parent,
         ...
         o.type, y.code, ty.name, ty.description,
         r.code, t.digital, rt.name, ot.label, rt.description, t.decimal,
