...
BEGIN
  ...
  LOOP
    ...
    IF r.code = 'create' THEN
      ...
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country created', 'EventCountryCreate();');
    END IF;

    IF r.code = 'open' THEN
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country opened', 'EventCountryOpen();');
    END IF;

    IF r.code = 'edit' THEN
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country edited', 'EventCountryEdit();');
    END IF;

    IF r.code = 'save' THEN
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country saved', 'EventCountrySave();');
    END IF;

    IF r.code = 'enable' THEN
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country enabled', 'EventCountryEnable();');
    END IF;

    IF r.code = 'disable' THEN
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country disabled', 'EventCountryDisable();');
    END IF;

    IF r.code = 'delete' THEN
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country will be deleted', 'EventCountryDelete();');
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
    END IF;

    IF r.code = 'restore' THEN
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country restored', 'EventCountryRestore();');
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
    END IF;

    IF r.code = 'drop' THEN
      PERFORM AddEvent(pClass, uEvent, r.id, 'Country will be destroyed', 'EventCountryDrop();');
      PERFORM AddEvent(pClass, uParent, r.id, 'Parent class events');
    END IF;
...
BEGIN
  -- Класс
  uClass := AddClass(pParent, pEntity, 'country', 'Country', false);
  PERFORM EditClassText(uClass, 'Страна', GetLocale('ru'));
  PERFORM EditClassText(uClass, 'Land', GetLocale('de'));
  PERFORM EditClassText(uClass, 'Pays', GetLocale('fr'));
  PERFORM EditClassText(uClass, 'Paese', GetLocale('it'));
  PERFORM EditClassText(uClass, 'País', GetLocale('es'));

  -- Тип
  uType := AddType(uClass, 'iso.country', 'ISO 3166', 'List of codes by ISO 3166.');
  PERFORM EditTypeText(uType, 'ISO 3166', 'Список кодов по ISO 3166.', GetLocale('ru'));
  PERFORM EditTypeText(uType, 'ISO 3166', 'Liste der Codes nach ISO 3166.', GetLocale('de'));
  PERFORM EditTypeText(uType, 'ISO 3166', 'Liste des codes selon ISO 3166.', GetLocale('fr'));
  PERFORM EditTypeText(uType, 'ISO 3166', 'Elenco dei codici secondo ISO 3166.', GetLocale('it'));
  PERFORM EditTypeText(uType, 'ISO 3166', 'Lista de códigos según ISO 3166.', GetLocale('es'));
...
BEGIN
  -- Сущность
  uEntity := AddEntity('country', 'Country');
  PERFORM EditEntityText(uEntity, 'Страна', null, GetLocale('ru'));
  PERFORM EditEntityText(uEntity, 'Land', null, GetLocale('de'));
  PERFORM EditEntityText(uEntity, 'Pays', null, GetLocale('fr'));
  PERFORM EditEntityText(uEntity, 'Paese', null, GetLocale('it'));
  PERFORM EditEntityText(uEntity, 'País', null, GetLocale('es'));
