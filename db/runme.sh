...
options=(
    ...
    "migrate (m): Apply patches + update"          # 6
    "database (db): [*] Creates an empty database" # 7
    "api (a): Drop and create api schema"          # 8
    "test (t): Run pgTAP tests"                    # 9
    "quit (q): Exit from this menu"                # 10
)
...
function _switch() {
    ...
    case $_reply in
        ...
        ""|"a"|"--api"|"api"|"8")
            ...
            ;;
        ""|"t"|"--test"|"test"|"9")
            "$(dirname "$(readlink -f "$0")")/test/run.sh"
            exit
            ;;
        ""|"q"|"quit"|"10")
            exit
