...
##### Код подтверждения
```http request
POST /api/v1/admin/verification/<agent>/code
```
...
##### Сверить
```http request
POST /api/v1/admin/verification/<agent>/try
```
...
##### Действие получить
```http request
POST /api/v1/admin/verification/get
```
...
##### Действие список
```http request
POST /api/v1/admin/verification/list
```
...
Где `<essence>` - это не абстрактная сущность смотрим на [дерево классов объекта](https://github.com/ufocomp/db-platform#%D0%B4%D0%B5%D1%80%D0%B5%D0%B2%D0%BE-%D0%BA%D0%BB%D0%B0%D1%81%D1%81%D0%BE%D0%B2-%D0%BE%D0%B1%D1%8A%D0%B5%D0%BA%D1%82%D0%B0) или выполняем [запрос](#сущности).
...
Какие именно _класс-таблицы_ участвуют в формировании набора данных можно посмотреть в [дереве классов объекта](https://github.com/ufocomp/db-platform#%D0%B4%D0%B5%D1%80%D0%B5%D0%B2%D0%BE-%D0%BA%D0%BB%D0%B0%D1%81%D1%81%D0%BE%D0%B2-%D0%BE%D0%B1%D1%8A%D0%B5%D0%BA%D1%82%D0%B0).
