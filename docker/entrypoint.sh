...
if ! [[ $PGWEB_DATABASE_URL ]]; then
  export PGWEB_DATABASE_URL=postgres://kernel:kernel@localhost:5432/crm
fi
...
/usr/sbin/$PROJECT_NAME -w $WORKER_PROCESSES & /usr/bin/pgweb --bind=0.0.0.0 --listen=8081 --skip-open
