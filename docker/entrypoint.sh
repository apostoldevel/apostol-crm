...
PROJECT_NAME="${PROJECT_NAME:-apostol-crm}"
...
set_env() {
  ...
  envsubst < conf/smtp.conf > /etc/"$PROJECT_NAME"/conf/smtp.conf
  envsubst < conf/default.json > /etc/"$PROJECT_NAME"/"$PROJECT_NAME".json
  pop_directory
}

init_app() {
  mkdir -p /etc/"$PROJECT_NAME"/oauth2 /etc/"$PROJECT_NAME"/sites
  push_directory /opt/"$PROJECT_NAME"
