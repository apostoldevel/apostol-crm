...
# or: cmake --build cmake-build-release --parallel $(nproc)
...
Backend listens on port 4977 inside container, mapped to 8082 on host.
...
│   Modules: AppServer, AuthServer, FileServer, WebServer, PGHTTP, WebSocketAPI, Heartbeat
├── Helper process (1)
│   Modules: PGFetch, PGFile
└── Background processes
    MessageServer, TaskScheduler, ReportServer
```

Workers connect to PostgreSQL (via pgbouncer) as `daemon`. Helpers/processes connect directly as `apibot`.

### Request Flow

```
HTTP request -> Apostol Worker (C++) -> ApostolModule -> libpq async query
-> rest.* dispatcher (PL/pgSQL) -> api.* CRUD -> kernel.* logic -> db.* tables
```

### Source Layout

- `src/app/main.cpp` -- entry point, `ApostolCRMApp` application class
- `src/lib/libapostol/` -- libapostol C++20 framework (git submodule)
- `src/modules/Workers/` -- HTTP request handler modules (cloned by `./configure`)
- `src/modules/Helpers/` -- background helper modules (PGFetch, PGFile)
- `src/processes/` -- independent background processes (cloned by `./configure`)

All modules and processes under `src/` (except libapostol) are separate git repos cloned by `./configure`.
...
- `conf/default.json` -- main JSON config (modules, processes, server, postgres connections)
...
Module/process enable/disable is in `conf/default.json` under `"module"` object.
...
Signals: TERM/INT (fast stop), QUIT (graceful stop), HUP (reload config + restart workers), WINCH (graceful worker shutdown). PID file: `/run/apostol-crm.pid`.

## Code Style

C++20 standard. Enforced by `.clang-format` (LLVM-based, 120 col, 4-space indent) and `.clang-tidy` (bugprone-*, modernize-*, performance-*, readability-* checks enabled at compile time).
